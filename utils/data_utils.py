...
#     args.num_labels = len(label_map)
#     args.tokenizer = tokenizer
#     client_train_data,client_test_data = partition_with_client_testdata(
#         client_datasets
#     )
#     return full_dataset, val_dataset, client_train_data, client_test_data

def partition_with_client_testdata(client_datasets):
    client_train_data = {}
    client_test_data = {}

    for cid, data in client_datasets:
        lengths = [int(0.8 * len(data)), len(data) - int(0.8 * len(data))]
        subset_train, subset_val = random_split(data, lengths)
        client_train_data[cid] = subset_train
        client_test_data[cid] = subset_val
